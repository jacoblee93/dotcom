  ...
  class Daemon {
    ...
    constructor() {
      ...
      process.on('exit', (code) => {
        ...
        console.log(`[Dotcom.Daemon] Shutdown: Exited with code ${code}`);

      });

      this.initializers = new ExecutionQueue();

      let initializersData = require(`${process.cwd()}/app/initializers.json`);
      (initializersData || []).forEach((initializer) => {
        initializer = require(`${process.cwd()}/initializers/${initializer}`);
    ...
    */
    start(port) {
      ...
      console.log('[Dotcom.Daemon] Startup: Initializing');
      ...
      this.initializers.exec((err) => {
        ...
        if ((process.env.NODE_ENV || 'development') === 'development') {
          ...
          this.watch('', (changes) => {
            changes.forEach(change => {
              console.log(`[Dotcom.Daemon] ${change.event[0].toUpperCase()}${change.event.substr(1)}: ${change.path}`);
            });
        ...
        console.log('[Dotcom.Daemon] Startup: Spawning HTTP Workers');
    ...
    */
    idle() {
      ...
      console.log(`[Dotcom.Daemon] Idle: Unable to spawn HTTP Workers, listening on port ${port}`);
    ...
    */
    error(error) {
      ...
      this._error = error;
      this._server = null;
      console.log(`[Dotcom.Daemon] ${error.name}: ${error.message}`);
